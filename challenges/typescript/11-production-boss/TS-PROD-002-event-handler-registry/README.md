# TS-PROD-002 — Derive an event handler registry
...
- Each member's `type` becomes a key of the registry.
- Its handler takes that member with the `type` discriminant removed, and returns `void`.
